...
class badDispatchFinder(object):
    ...
    def __init__(self, maxTime, tasksDict):
        self.timeBlocks = [0 for i in range(int(maxTime / RESOLUTION_TASK_DISCOVER))]
        for task in list(tasksDict.values()):
            self.addTask(task)
        return

    def addTask(self, taskInfo):
        for i in range(int(taskInfo[0] / RESOLUTION_TASK_DISCOVER), int(taskInfo[1][0] / RESOLUTION_TASK_DISCOVER)):
            self.timeBlocks[i] += 1
...
def synchroWorkersTime(workerFileBeginPath):
    ...
    for i in range(1, len(msgTreeList)):
        j = 0
