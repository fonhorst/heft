...
class CommThread(AbstractCommThread):
    ...
    def run(self):
        ...
        def mpiSend(msg, dest):
            ...
            arrayBuf.fromstring(pickle.dumps(msg, pickle.HIGHEST_PROTOCOL))
        ...
        while working:
            ...
            for i, reqTuple in enumerate(lRecvWaiting):
                if reqTuple[1].Test():
                    countRecv += 1
                    dataS = pickle.loads(reqTuple[0].tostring())
                    if self.traceMode:
            ...
            lRecvWaiting = [d for d in lRecvWaiting if not d is None]
            ...
            lSendWaiting = [d for d in lSendWaiting if not d[0].Test()]
