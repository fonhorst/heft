...
import functools
from heft.algs.gsa.vm_cgsa.cgsa import vm_run_cgsa, Env
...
class CgsaVmWrapper:
    ...
    def __call__(self, fixed_schedule_part, initial_schedule, current_time=0, initial_population=None):
        ...
        print("CGSA_START")
        best, pops, logbook, initial_pops, hall, vm_series = vm_run_cgsa(**kwargs)
...
def create_cgsa(_wf, rm, estimator,
                     init_sched_percent,
                     log_book, stats,
                     alg_params):

    return CgsaVmWrapper(_wf, rm, estimator,
                        init_sched_percent,
                        log_book, stats,
                        alg_params)
...
cgsa_exp = partial(do_cpso, alg_builder=create_cgsa)

if __name__ == "__main__":
    p = deepcopy(BASE_PARAMS)
    # test_run(ga_exp, BASE_PARAMS)
    changing_reliability_run(cgsa_exp, RELIABILITY, INDIVIDUALS_COUNTS, REPEAT_COUNT, WF_NAMES, BASE_PARAMS)
