...
class Checkpoint(object):
    ...
    def dump(self, file):
        ...
        *filestream*.
        ...
        """
        self.values = dict.fromkeys(iter(self.objects.keys()))
...
class Statistics(object):
    ...
    def register(self, name, function):
        ...
        self.data[name] = [[] for _ in range(self.dim)]
    ...
    def update(self, seq, index=0, add=False):
        ...
        try:
            ...
            values = list(zip(*(self.key(elem) for elem in seq)))
        except TypeError:
        ...
        for key, func in self.functions.items():
            try:
                self.data[key][index].append(list(map(func, values)))
            except IndexError:
                if add and index == len(self.data[key]):
                    self.data[key].append([list(map(func, values))])
                else:
                    raise
    ...
    def update(self, population):
        ...
        *population* (if they are better). The size of the hall of fame is
        ...
        if len(self) < self.maxsize:
            ...
            self.keys = list(map(attrgetter("fitness"), reversed(self.items)))
        else:
