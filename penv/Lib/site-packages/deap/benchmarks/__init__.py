...
def dtlz2(individual, obj):
    ...
    f.extend((1.0+g) * reduce(mul, (cos(0.5*xi*pi) for xi in xc[:m-1]), 1) * sin(0.5*xc[m]*pi) for m in reversed(range(obj-1)))
    return f
...
def dtlz4(individual, obj, alpha):
    ...
    f.extend((1.0+g) * reduce(mul, (cos(0.5*xi**alpha*pi) for xi in xc[:m-1]), 1) * sin(0.5*xc[m]**alpha*pi) for m in reversed(range(obj-1)))
    return f
