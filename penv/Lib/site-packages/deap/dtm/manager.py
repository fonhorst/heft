...
class TaskQueue(object):
    ...
    def getLoad(self):
        ...
        for tid, tObj in list(self._tDict.items()):
    ...
    def getTasksIDsWithExecTime(self, execTimeWanted, maxDiff= -1.):
        ...
        if len(returnList) == 0 and maxDiff == -1 and len(self._tDict) > 1:
            it = [x for x in list(self._tDict.items()) if self._getTimeInfoAbout(x[1]) != 0]
            if len(it) != 0:
...
class Control(object):
    ...
    def _doCleanUp(self):
        ...
        if self.commThread.isRootWorker:
            if self.printExecSummary:
                ...
                for target, times in list(self.tasksStats.items()):
    ...
    def _logTaskStatChange(self):
        ...
        for taskTarget, taskInfo in list(self.tasksStats.items()):
            try:
    ...
    def _updateStats(self, msg):
        ...
        for key, val in list(msg.targetsStats.items()):
                ...
                try:
                    ...
                    if self.traceMode:
                        ...
                        for k in newTask.kwargs[0]:
                            newTaskTarget.set("kwarg_" + str(k), str([newTask.kwargs[i][k] for i in range(len(newTask.kwargs))]))
    ...
    def map(self, function, *iterables, **kwargs):
        ...
        for indexBegin in range(0, len(zipIterable), chunkSize):
            ...
            task = TaskContainer(tid = self.idGenerator.tid,
                                    ...
                                    taskIndex = [i for i in range(indexBegin, topBound)],
                                    taskRoute = [],
                                    creationTime = time.time(),
                                    target = [function for i in range(indexBegin, topBound)],
                                    args = [zipIterable[i] for i in range(indexBegin, topBound)],
                                    kwargs = [kwargs for i in range(indexBegin, topBound)],
                                    threadObject = None,
                                    taskState = DTM_TASK_STATE_IDLE,
                                    lastSubTaskDone = None)
        ...
        self.waitingThreadsLock.acquire()
        if currentId not in list(self.waitingThreads.keys()):
    ...
    def imap(self, function, *iterables):
        ...
        asyncResults = [self.apply_async(function, *zipIterable[i]) for i in range(len(zipIterable))]
    ...
    def repeat(self, function, n, *args, **kwargs):
        ...
        for indexBegin in range(0, n, chunkSize):
            ...
            task = TaskContainer(tid = self.idGenerator.tid,
                                    ...
                                    creationTime = time.time(),
                                    target = [function for i in range(indexBegin, topBound)],
                                    args = [args for i in range(indexBegin, topBound)],
                                    kwargs = [kwargs for i in range(indexBegin, topBound)],
                                    threadObject = None,
                                    taskState = DTM_TASK_STATE_IDLE,
                                    lastSubTaskDone = None)
    ...
    def waitAll(self, reqList = []):
        ...
        self.waitingThreadsLock.acquire()
        if currentId not in list(self.waitingThreads.keys()) or len(self.waitingThreads[currentId].rWaitingDict) == 0:
            self.waitingThreadsLock.release()
            return
...
class DtmThread(threading.Thread):
    ...
    def run(self):
        ...
        if self.tid in list(self.control.waitingThreads.keys()):
